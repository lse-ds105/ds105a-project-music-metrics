...
```{r}
file.exists("visualisation.csv")
```


```{r}
df <- readr::read_csv('./visualisation.csv')
```
...
g <- ggplot(data = corr_df2, aes(x = rowname, y = column, fill = value)) +
  ...
  geom_text(aes(label = rounded_value), fill = "white", size = 3.5) +  # Add labels
  scale_fill_distiller(palette = "Spectral") +  # Use a color gradient for the fill
  labs(title = 'Correlation Matrix', x = '', y = '') +  # Add titles and remove axis labels
